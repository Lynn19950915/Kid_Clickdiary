<?php
    ...
    if(isset($_FILES["csv_file1"])){
        ...
        exec("Rscript schema.R ".$_POST["project_id1"]." ".$_POST["fname1"]);
    ...
    if(isset($_FILES["csv_file2"])){
        ...
        while(($data=fgetcsv($handle))!==false){
            $sql12="INSERT INTO `sample` VALUES(:v1, :v2)";
        ...
        $sql13="UPDATE `project` SET active=3 WHERE project_id= :v1";
		$stmt=$db->prepare($sql13);
		$stmt->bindParam(":v1", $_POST["project_id2"]);
		$stmt->execute();
        
        $final=$_POST["project_id2"]."final";
        $sql14="CREATE TABLE `:v1`(`record_id` INT AUTO_INCREMENT, `sample_id` INT NOT NULL, `time` TIMESTAMP NOT NULL, `record` VARCHAR(10000) NOT NULL, PRIMARY KEY(`record_id`))";
            ...
            $("#uploadCsvForm").on("submit", function(event){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        $.confirm({
                            title: "",
                            content: "上傳成功！點按 <i class='fas fa-search-plus' style='color: #003B83'></i> 即可預覽問卷",
                            buttons:{
                                "OK": function(){
