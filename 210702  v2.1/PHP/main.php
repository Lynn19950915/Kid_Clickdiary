<?php
    ...
    if(isset($_POST["searchProject"])){
        ...
        if($_SESSION["acc_info"]["level"]==1){
            $sql7="SELECT * FROM `project` LEFT JOIN `account` ON project.id=account.id WHERE project_name LIKE '%$search%' or name LIKE '%$search%'";
            ...
            $sql8="SELECT * FROM `project` LEFT JOIN `account` ON project.id=account.id WHERE account.id= :v1 and (project_name LIKE '%$search%' or name LIKE '%$search%')";
    ...
    if(isset($_FILES["csv_file1"])){
        ...
        while(($data=fgetcsv($handle))!=false){
            if($count==1){
                ...
                if($data[0]==""){
                    ...
                    echo "Invalid Q_sn";
                    exit();
                }else if(!ctype_digit($data[3])|$data[3]>3){
                    echo "Invalid Type";
                    exit();
        ...
        if($output==array()){
            ...
            $sql9="UPDATE `project` SET active=2 WHERE project_id= :v1";
            $stmt=$db->prepare($sql9);
            $stmt->bindParam(":v1", $_POST["project_id1"]);
            $stmt->execute();

            $beta=$_POST["project_id1"]."beta";
            $sql10="DROP TABLE IF EXISTS `:v1`";
            $stmt=$db->prepare($sql10);
            $stmt->bindParam(":v1", $beta);
            $stmt->execute();

            $sql11="CREATE TABLE `:v1`(`record_id` INT AUTO_INCREMENT, `time` TIMESTAMP NOT NULL, `record` VARCHAR(10000) NOT NULL, PRIMARY KEY(`record_id`))";
    ...
    if(isset($_FILES["csv_file2"])){
        ...
        $filepath="C:/***"."/".$files[0]["name"];
        $handle=fopen($filepath, "r");
        $sql12="DELETE FROM `sample` WHERE project_id= :v1";
        ...
        while(($data=fgetcsv($handle))!=false){
            if($count==1){
                ...
                if($data[0]==""){
                    ...
                    $sql13="INSERT INTO `sample` VALUES(:v1, :v2, :v3)";
        ...
        $sql14="UPDATE `project` SET active=3 WHERE project_id= :v1";
		$stmt=$db->prepare($sql14);
		$stmt->bindParam(":v1", $_POST["project_id2"]);
		$stmt->execute();
        
        $final=$_POST["project_id2"]."final";
        $sql15="CREATE TABLE `:v1`(`record_id` INT AUTO_INCREMENT, `sample_id` INT NOT NULL, `time` TIMESTAMP NOT NULL, `record` VARCHAR(10000) NOT NULL, PRIMARY KEY(`record_id`))";
    ...
    <script>
		$(document).ready(function(){            
                ...
                success: function(data){
                    ...
                    for($i=0; $i<data.length; $i++){
                        ...
                        if(data[$i].active==0|data[$i].active==1){
                            ...
                            append_row.insertCell(7).innerHTML="<a href='./download1.php?project_id="+data[$i]['project_id']+"'><button class='btn' style='color: #FFFFFF; background-color: #EF82A0'><i class='fas fa-download'></i></button></a>";
                        }
                    ...
                    success: function(data){
                        ...
                        for($j=0; $j<data.length; $j++){
                            ...
                            if(data[$j].active==0|data[$j].active==1){
                                ...
                                append_row.insertCell(7).innerHTML="<a href='./download1.php?project_id="+data[$j]['project_id']+"'><button class='btn' style='color: #FFFFFF; background-color: #EF82A0'><i class='fas fa-download'></i></button></a>";
                            }
                            ...
                            if(data[$j].active==0|data[$j].active==1|data[$j].active==2){
                                ...
                                append_row.insertCell(10).innerHTML="<a href='./download2.php?project_id="+data[$j]['project_id']+"'><button class='btn btn-danger'><i class='fas fa-download'></i></button></a>";
                            }
                        }
            ...
            $("#uploadCsvForm").on("submit", function(event){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        if(data=="Invalid Format"){
                        ...
                        }else if(data=="Header Q_id"){
                            $("#submit2").attr("disabled", false);
                            $.alert({
                                title: "<i class='fas fa-times-circle' style='color: red'></i> 欄位名稱錯誤",
                                content: "<b style='color: blue'>A 欄</b> 表頭應為 <b style='color: red'>[q_id]</b>，請檢查欄位是否存在或順序有誤！",
                            })
                            ...
                            $.alert({
                                title: "<i class='fas fa-times-circle' style='color: red'></i> 必填欄位缺漏",
                                content: "系統偵測到您檔案中的 <b style='color: red'>[skip]</b> 欄位有漏填，此為必填欄位，每行都需填寫唷！",
                            })
                            ...
                            $.alert({
                                title: "<i class='fas fa-times-circle' style='color: red'></i> 填寫格式錯誤",
                                content: "系統偵測到您檔案中的 <b style='color: red'>[type]</b> 欄位有錯誤，合理的輸入值為 <b style='color: blue'>0,1,2,3</b>，請檢查修正！",
                            })
            ...
            $("#submitCsvForm").on("submit", function(event){
                ...
                var project_id2=$("input[name='project_id2']").val();
                var filename2=$("#csv_file2").get(0).files[0].name;
                
                var data=new FormData();
                data.append("csv_file2", file2);
                data.append("project_id2", project_id2);
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        }else if(data=="Header Sample_id"){
                            $("#submit3").attr("disabled", false);
                            $.alert({
                                title: "<i class='fas fa-times-circle' style='color: red'></i> 欄位名稱錯誤",
                                content: "<b style='color: blue'>A 欄</b> 表頭應為 <b style='color: red'>[sample_id]</b>，請檢查欄位是否存在或順序有誤！",
                            })
...
<body>
    ...
    <div id="submit_csv" class="modal fade" style="top: 20%">
        <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body" style="padding-bottom: 1em">
                <form id="submitCsvForm"><br>
                    ...
                    <p class="row" style="font-size: 0.8em; margin: 2.5% auto"><b><i class='fa fa-info-circle'></i> 此為專案上線後，可訪問連結的樣本清單。若有疑問請聯繫管理人員。</b></p>
                    <div class="row">
                        <div class="col-sm-4 pl-0 pr-0">
                            <div class="input-group-text">B. 樣本檔案：</div>
                        </div>
                        <div class="col-sm-8 pl-0 pr-0">
            ...
            <div class="card-body show" style="padding: 2% 10% 0% 10%">
                ...
                <table class="table rule" style="text-align: center">
                    ...
                    <tr>
                        ...
                        <td style="line-height: 2em; vertical-align: middle"><i class="fas fa-search-plus" style="font-size: 1.375em; color: #003B83; vertical-align: middle"></i>　<button class="btn" style="color: #FFFFFF; background-color: #EF82A0"><i class="fas fa-download"></i></button>　<button class="btn btn-danger"><i class="fas fa-download"></i></button>　</td>
                    </tr>
                    <tr>
