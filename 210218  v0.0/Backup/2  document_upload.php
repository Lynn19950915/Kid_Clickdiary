 ...
 */
function uploadFile($fileInfo, $allowExt = array('csv', 'json'), $maxSize = 209715200, $flag = true, $uploadPath = '***/uploads') {
	// 存放錯誤訊息
    $res = array();
		
    // 取得上傳檔案的擴展名
    $ext = pathinfo($fileInfo['name'], PATHINFO_EXTENSION);
    if(explode(".", $fileInfo['name'])[1] == "json"){
        $destination = $uploadPath . '/' . $fileInfo['name'];
    }else{
       $destination = $uploadPath . '/' . $fileInfo['name']; 
        // $destination = 'https://capi.geohealth.tw/root/uploads/'.$fileInfo['name'];
    } 
